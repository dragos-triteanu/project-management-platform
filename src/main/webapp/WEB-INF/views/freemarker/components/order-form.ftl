...
<div id="order-details">
    ...
    <#if userRole == "CLIENT">
        <div class="form-group">
            ...
            <#elseif order.orderStatus="INPROGRESS">
                <div class="alert alert-info col-md-6 col-xs-6" role="alert" id="assignedTo" name="content">
                    <p>Comanda este in desfasurare. Mai aveti de achitat suma de ${paymentDetails.amountDue} RON in maxim ${3 - paymentDetails.nrOfEffectuated0fTransactions} rate.</p>
            ...
            <#elseif order.orderStatus="DONE">
                <div class="alert alert-info col-md-6 col-xs-6" role="alert" id="assignedTo" name="content">
                    ...
                    <#if paymentDetails.amountDue != 0>
                        <p>Pentru a intra in posesia acestia va rugam sa achitati suma de ${paymentDetails.amountDue} RON in maxim ${3 - paymentDetails.nrOfEffectuated0fTransactions} rate.</p>
                    </#if>
                </div>
            </#if>
        </div>
    ...
    <#if userRole=="CONSULTANT">
        ...
        <#elseif order.orderStatus == 'INPROGRESS'>
            <div class="alert alert-info container col-md-7 col-xs-7" role="alert">
                <p>Pentru a confirma finalizarea comenzii va rugam apasati
                    <button id="closeOrder" class="btn btn-info btn-xs">Terminat</button></p>
            </div>
        <#elseif order.orderStatus == 'DONE'>
            <div class="alert alert-info container col-md-7 col-xs-7" role="alert">
                <p>Comanda este finalizata cu succes.</p>
            </div>
        </#if>
    </#if>
    ...
    <#if userRole=="ADMINISTRATOR">
        <div class="form-group">
            ...
            <#elseif order.orderStatus=="ASSIGNED" || order.orderStatus=="INPROGRESS" || order.orderStatus=="DONE">
                <div class="col-md-6 col-xs-6">
        ...
        <#if order.orderStatus=="INPROGRESS" || order.orderStatus=="DONE">
            <div class="form-group">
                <div class="col-md-2 col-xs-2">
                    <label for="payment-details"  class="control-label">Detalii plata:</label>
                </div>
                <div class="col-md-6 col-xs-6">
                    <#if paymentInfo??>
                        <#if paymentInfo.amountDue == 0>
                            <div class="alert alert-info" role="alert"><p>Comanda a fost platita integral.</p></div>
                        <#else>
                            <div class="alert alert-info" role="alert"><p>Rest plata ${paymentInfo.amountDue} RON in maxim ${3 - paymentInfo.nrOfEffectuated0fTransactions} rate.</p></div>
                        </#if>
                    </#if>
                </div>
            </div>
